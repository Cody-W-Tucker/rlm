defmodule Rlm.Engine.Prompt.Base do
  ...
  def system_prompt(settings, iteration, run_state, strategy_constraints) do
    ...
    - If filename/path structure is not informative, use `grep_files()` or `grep_open()` to search for concrete behavioral markers, local examples, and contradiction cues rather than abstract theory labels.
    ...
    - For multi-file file-backed questions, follow this sequence before finalizing: scout corpus shape, search behavioral markers, read surrounding passages, form a working hypothesis, run one contradiction or disconfirmation search, then answer from the updated evidence set.
    ...
    - For synthesis-heavy questions over messy human text, include concrete observed examples, a competing interpretation or contradiction check, and then the final inference.
